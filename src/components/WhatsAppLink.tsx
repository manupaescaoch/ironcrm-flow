...
const DEFAULT_MESSAGE = "Olá! Aqui é da EVO CLUB. Tudo bem?";
...
export function WhatsAppLink({ phone, className, showIcon = true, iconOnly = false, message = DEFAULT_MESSAGE }: WhatsAppLinkProps) {
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "transition-colors focus:outline-none focus:ring-2 focus:ring-green-500/20",
          className
        )}
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        "inline-flex items-center gap-1.5 text-foreground hover:text-green-600",
    ...
    >
      <span>{phone}</span>
      {showIcon && <WhatsAppIcon className="text-green-600" />}
    </button>
  );
}
