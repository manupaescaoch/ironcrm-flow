...
ALTER TABLE public.leads
  ...
  ADD COLUMN IF NOT EXISTS nivel_interesse_atualizado_em timestamptz,
  ADD COLUMN IF NOT EXISTS nivel_interesse_atualizado_por text;

ALTER TABLE public.leads
  DROP CONSTRAINT IF EXISTS leads_nivel_interesse_check;
ALTER TABLE public.leads
  ADD CONSTRAINT leads_nivel_interesse_check
  CHECK (nivel_interesse IS NULL OR nivel_interesse IN ('alto','medio','baixo'));

CREATE INDEX IF NOT EXISTS idx_leads_nivel_interesse ON public.leads(nivel_interesse);

-- 2) Atualiza enforce_leads_update_permissions para permitir alteração desses 3 campos
...
BEGIN
  ...
  -- Permite alteração de nivel_interesse por qualquer usuário com acesso à unidade do lead
  ...
  THEN
    ...
    END IF;
    -- Permitido — continua para validar outros campos
  END IF;
...
-- 3) Trigger: limpa nivel_interesse quando lead vai para perdido
CREATE OR REPLACE FUNCTION public.clear_nivel_interesse_on_perdido()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
BEGIN
  IF NEW.status_funil = 'perdido' AND OLD.status_funil IS DISTINCT FROM 'perdido' THEN
    NEW.nivel_interesse := NULL;
    NEW.nivel_interesse_atualizado_em := now();
    NEW.nivel_interesse_atualizado_por := 'SISTEMA (perdido)';
  END IF;
  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_clear_nivel_interesse_on_perdido ON public.leads;
CREATE TRIGGER trg_clear_nivel_interesse_on_perdido
  BEFORE UPDATE ON public.leads
  FOR EACH ROW
  EXECUTE FUNCTION public.clear_nivel_interesse_on_perdido();